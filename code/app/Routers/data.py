...
def save(request: RequestParam):
    ...
    for n in range(limit): 
        params = {"q": search, "hl": "ko", "start": n-1*10}
        ...
        articleList = soup.find_all("div", "kvH3mc BToiNc UK95Uc")
        
        fileNameList = ["./"+search+".csv"]
        fileNameList.append("./Log/"+search+"_"+datetime.datetime.now().strftime('%Y-%m-%d')+".csv")
        
    return pd.DataFrame(saveFile(fileNameList, articleList))

def saveFile(fileNameList, articleList):
